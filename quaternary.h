...
public:
    Quaternary(Expression *left, Expression *right, Expression *middle, Expression *last) : SubExpression(left, right, middle, last) {
    }

    double evaluate() {return left->evaluate() < 0 ? right->evaluate()
    : (left->evaluate() == 0 ? middle->evaluate() : last->evaluate()); }
};
